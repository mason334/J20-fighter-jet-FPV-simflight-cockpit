void cal_att_cord(){
  ...
  x0 = r0*cosa + x_center;
  y_0 = r0*sina + y_center;
  vx = level_pixel*cosa;
  vy = level_pixel*sina;

  center_degree = (pitch)/PI*180;
  base_vect = round(center_degree/level_angle); // baseline中点到x0,y_0的距离  
  //0度地平线坐标
  xa1 = x0+cosb*r1;
  ya1 = y_0+sinb*r1;
  xa2 = x0+cosb*r2;
  ...
  //屏幕中心baseline坐标，也就是第四行坐标：att_points[3]
  ...
  for (int i=0; i < 7; i++) {      
  int j = i-3;//j用于标记姿态线距离baseline姿态线的距离, i用于标记姿态线，最pitch最高的一根编号为0，最低的编号为6，baseline编号为3
